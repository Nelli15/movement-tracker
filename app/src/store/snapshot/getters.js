...
export function lastUpdated (state) {
  if (
    state.snapshot.last_modified &&
    state.snapshot.last_modified !== 'Error: No Date Available'
  ) {
    var date = state.snapshot.last_modified.toDate()
...
export function backgroundColor (state) {
  if (state.snapshot) {
      if (state.snapshot.backgroundColor) {
        return state.snapshot.backgroundColor
      }
  }
  return 'white'
}
  ...
  if (state.snapshot) {
      if (state.snapshot.backgroundColor) {
        let r = 0,
          g = 0,
          b = 0
        const h = state.snapshot.backgroundColor
        if (h.length === 7) {
...
export function modOpts (state) {return state.roles !== {} ? toArray(state.mods).sort((a, b) => {
      return a.label > b.label ? 1 : a.label < b.label ? -1 : 0
    }) : []}
export function complexOpts (state) { return state.roles !== {} ? toArray(state.complexStats).sort((a, b) => {
      return a.label > b.label ? 1 : a.label < b.label ? -1 : 0
    }) : []}
export function calcOpts (state) { return state.roles !== {} ? toArray(state.calcStats).sort((a, b) => {
      return a.label > b.label ? 1 : a.label < b.label ? -1 : 0
    }) : []}
...
export function treeSorted (state) { 
  // console.log(state.currentTree, state.trees[state.currentTree.id], state.trees[state.currentTree.id].tree)
