...
describe('Public documents', () => {
  ...
  //     await setDoc(doc(context.firestore(), 'app-users/foobar'), { foo: 'bar' });
  //   });
  ...
  it('should let anyone read any version doc', async function() {
    ...
    await testEnv.withSecurityRulesDisabled(async function (context) {
      await setDoc(doc(context.firestore(), 'version-info/foobar'), { foo: 'bar' });
    });
  ...
  it('should let anyone read any help doc', async function() {
    // Setup: Create documents in DB for testing (bypassing Security Rules).
    await testEnv.withSecurityRulesDisabled(async (context) => {
      await setDoc(doc(context.firestore(), 'help/foobar'), { foo: 'bar' });
    });
  ...
  describe('Movement Doc', () => {
    it('Can read doc if user is in the role subcollection', async () => {
      await testEnv.withSecurityRulesDisabled(async (context) => {
        ...
        await setDoc(doc(firestore, `/movements/${movId}`), { movId })
        await setDoc(doc(firestore, `/movements/${movId}/users/${myId}`), { uid: myId })
      })
    ...
    it('Can update doc if user is owner', async () => {
      await testEnv.withSecurityRulesDisabled(async (context) => {
        const firestore = context.firestore()
        await setDoc(doc(firestore, `/movements/${movId}`), { movId })
        await setDoc(doc(firestore, `/movements/${movId}/users/${myId}`), { uid: myId, role: 'owner' })
      })
    ...
    it('Can update doc if user has permission', async () => {
      await testEnv.withSecurityRulesDisabled(async (context) => {
        const firestore = context.firestore()
        await setDoc(doc(firestore, `/movements/${movId}`), { movId })
        await setDoc(doc(firestore, `/movements/${movId}/users/${myId}`), { uid: myId, role: 'otherRole' })
        await setDoc(doc(firestore, `/movements/${movId}/user-role-definitions/otherRole`), { rules: { movement_edit: true} })
      })
      await assertSucceeds(updateDoc(doc(myApp, `/movements/${movId}`), { movementName: 'abc'}))
    })
    ...
    it('Can delete doc if user has permission', async () => {
      await testEnv.withSecurityRulesDisabled(async (context) => {
        ...
        await setDoc(doc(firestore, `/movements/${movId}`), { movId })
        await setDoc(doc(firestore, `/movements/${movId}/users/${myId}`), { uid: myId, role: 'otherRole' })
        await setDoc(doc(firestore, `/movements/${movId}/user-role-definitions/otherRole`), { rules: { movement_delete: true} })
      })
      await assertSucceeds(deleteDoc(doc(myApp, `/movements/${movId}`)))
    })
    ...
    for(let type of styleTypes) {
      describe(`${type.label}:`, () => {
        ...
        for(let access of accessTests) {
          let tests = [
            { label: `Can ${access.type} doc if user is owner`, assert: assertSucceeds, user: async (firestore) => await setDoc(doc(firestore, `/movements/${movId}/users/${myId}`), { uid: myId, role: 'owner' }) },
            { label: `Can ${access.type} doc if user has permission`, assert: assertSucceeds, permissions: { rules: { [access.permission]: true} } , user: async (firestore) => await setDoc(doc(firestore, `/movements/${movId}/users/${myId}`), { uid: myId, role: 'otherRole' })},
            ...
            it(test.label, async () => {
              await testEnv.withSecurityRulesDisabled(async (context) => {
                ...
                if(['read', 'update', 'delete'].includes(access.type)) {
                  await setDoc(doc(firestore, testDoc), { label: 'abc', type: type.type })
                }
                if(!!test.permissions){
                  if(test.label === 'Can create doc if user has permission') {test.permissions}
                  await setDoc(doc(firestore, `/movements/${movId}/user-role-definitions/otherRole`), test.permissions)
                }
                if(!!test.user){
                  await test.user(firestore)
                }
              })
  ...
  describe('Lists', () => {
    ...
    for(let access of accessTests) {
      let tests = [
        { label: `Can ${access.type} doc if user is owner`, assert: assertSucceeds, user: async (firestore) => await setDoc(doc(firestore, `/movements/${movId}/users/${myId}`), { uid: myId, role: 'owner' }) },
        { label: `Can ${access.type} doc if user has valid role`, assert: assertSucceeds, permissions: { rules: {  }} , user: async (firestore) => await setDoc(doc(firestore, `/movements/${movId}/users/${myId}`), { uid: myId, role: 'otherRole' })},
        // { label: `Can't ${access.type} if user is not a member of the movement`, assert: assertFails }
      ]
      for(let test of tests) {
        it(test.label, async () => {
          await testEnv.withSecurityRulesDisabled(async (context) => {
            ...
            // await setDoc(doc(firestore, `/movements/${movId}`), { movId })
            if(['read', 'update', 'delete'].includes(access.type)) {
              await setDoc(doc(firestore, testDoc), { name: 'abc' })
            }
            if(!!test.permissions){
              await setDoc(doc(firestore, `/movements/${movId}/user-role-definitions/otherRole`), test.permissions)
            }
      ...
      for(let test of tests) {
        it(test.label, async () => {
          await testEnv.withSecurityRulesDisabled(async (context) => {
            ...
            // await setDoc(doc(firestore, `/movements/${movId}`), { movId })
            if(['read', 'update', 'delete'].includes(access.type)) {
              await setDoc(doc(firestore, testDoc), { label: 'abc' })
            }
            if(!!test.permissions){
              await setDoc(doc(firestore, `/movements/${movId}/user-role-definitions/otherRole`), test.permissions)
            }
  ...
  describe('Snapshots Doc', () => {
    ...
    for(let access of accessTests) {
      let tests = [
        { label: `Can ${access.type} doc if user is owner`, assert: assertSucceeds, skip: ['delete'], user: async (firestore) => await setDoc(doc(firestore, `/movements/${movId}/users/${myId}`), { uid: myId, role: 'owner' }) },
        { label: `Can ${access.type} doc if user has permission`, assert: assertSucceeds, skip: ['delete'], permissions: { rules: { [access.permission]: true} } , user: async (firestore) => await setDoc(doc(firestore, `/movements/${movId}/users/${myId}`), { uid: myId, role: 'otherRole' })},
        { label: `Can't ${access.type} if user is not a member of the movement`, assert: assertFails, skip: ['delete'] },
        { label: `Can't ${access.type} doc if user is owner`, assert: assertSucceeds, skip: ['read', 'update', 'create'], user: async (firestore) => await setDoc(doc(firestore, `/movements/${movId}/users/${myId}`), { uid: myId, role: 'owner' }) },
        { label: `Can't ${access.type} doc if user has permission`, assert: assertSucceeds, skip: ['read', 'update', 'create'], permissions: { rules: { [access.permission]: true} } , user: async (firestore) => await setDoc(doc(firestore, `/movements/${movId}/users/${myId}`), { uid: myId, role: 'otherRole' })},
          ...
          for(let test of tests) {
            it(test.label, async () => {
              await testEnv.withSecurityRulesDisabled(async (context) => {
                ...
                // await setDoc(doc(firestore, `/movements/${movId}`), { movId })
                if(['read', 'update', 'delete'].includes(access.type)) {
                  await setDoc(doc(firestore, l_testDoc), { name: 'abc' })
                }
                if(!!test.permissions){
                  await setDoc(doc(firestore, `/movements/${movId}/user-role-definitions/otherRole`), test.permissions)
                }
  ...
  describe('Users Doc', () => {
    ...
    // for(let access of accessTests) {
      let tests = [
        { label: `Can read doc if user is owner`, assert: assertSucceeds, user: async (firestore) => await setDoc(doc(firestore, `/movements/${movId}/users/${myId}`), { uid: myId, role: 'owner' }), type: 'read', func: accessTests.read.func},
        { label: `Can read doc if user has permission`, assert: assertSucceeds, permissions: { rules: { [accessTests.read.permission]: true} } , user: async (firestore) => await setDoc(doc(firestore, `/movements/${movId}/users/${myId}`), { uid: myId, role: 'otherRole' }), type: 'read', func: accessTests.read.func},
        { label: `Can read if doc is the user accessing it`, assert: assertSucceeds, user: async (firestore) => await setDoc(doc(firestore, `/movements/${movId}/users/${theirId}`), { uid: theirId, role: 'otherRole' }), type: 'read', func: () => accessTests.read.func(theirApp) },
        { label: `Can't read if user is not a member of the movement`, assert: assertFails, type: 'read', func: accessTests.read.func},
        { label: `Can create doc if user is owner`, assert: assertSucceeds, user: async (firestore) => await setDoc(doc(firestore, `/movements/${movId}/users/${myId}`), { uid: myId, role: 'owner' }), type: 'create', func: accessTests.create.func},
        { label: `Can create doc if user has permission`, assert: assertSucceeds, permissions: { rules: { [accessTests.create.permission]: true} } , user: async (firestore) => await setDoc(doc(firestore, `/movements/${movId}/users/${myId}`), { uid: myId, role: 'otherRole' }), type: 'create', func: accessTests.create.func},
        { label: `Can update doc if user is owner`, assert: assertSucceeds, user: async (firestore) => await setDoc(doc(firestore, `/movements/${movId}/users/${myId}`), { uid: myId, role: 'owner' }), type: 'update', func: accessTests.update.func},
        { label: `Can't update doc if owner is changing themself`, assert: assertFails, user: async (firestore) => await setDoc(doc(firestore, `/movements/${movId}/users/${theirId}`), { uid: theirId, role: 'owner' }), type: 'update', func: () => accessTests.update.func(theirApp)},
        { label: `Can update doc if user has permission`, assert: assertSucceeds, permissions: { rules: { [accessTests.update.permission]: true} } , user: async (firestore) => await setDoc(doc(firestore, `/movements/${movId}/users/${myId}`), { uid: myId, role: 'otherRole' }), type: 'update', func: accessTests.update.func},
        { label: `Can't update doc if non-owner is changing an owner`, assert: assertFails, user: async (firestore) => await setDoc(doc(firestore, `/movements/${movId}/users/${myId}`), { uid: myId, role: 'otherRole' }), type: 'update', func: accessTests.update.func, testUserRole: 'owner'},
        ...
        { label: `Can delete doc if user is owner`, assert: assertSucceeds, user: async (firestore) => await setDoc(doc(firestore, `/movements/${movId}/users/${myId}`), { uid: myId, role: 'owner' }), type: 'delete', func: accessTests.delete.func},
        ...
        { label: `Can delete doc if user has permission`, assert: assertSucceeds, permissions: { rules: { [accessTests.delete.permission]: true} } , user: async (firestore) => await setDoc(doc(firestore, `/movements/${movId}/users/${myId}`), { uid: myId, role: 'otherRole' }), type: 'delete', func: accessTests.delete.func},
        { label: `Can't update doc if non-owner is changing an owner`, assert: assertFails, user: async (firestore) => await setDoc(doc(firestore, `/movements/${movId}/users/${myId}`), { uid: myId, role: 'otherRole' }), type: 'update', func: accessTests.update.func, testUserRole: 'owner'},
      ]
      for(let test of tests) {
        it(test.label, async () => {
          await testEnv.withSecurityRulesDisabled(async (context) => {
            ...
            if(['read', 'update', 'delete'].includes(test.type)) {
              await setDoc(doc(firestore, testDoc), { name: 'abc', role: test.testUserRole ? test.testUserRole : 'otherRole' })
            }
            if(!!test.permissions){
              await setDoc(doc(firestore, `/movements/${movId}/user-role-definitions/otherRole`), test.permissions)
            }
    ...
    describe('Graphs Doc',  () => {
      ...
      for(let access of accessTests) {
        let tests = [
          ...
          { label: `Can ${access.type} doc if user has permission`, assert: assertSucceeds, permissions: { rules: { [access.permission]: true} } , user: async (firestore) => await setDoc(doc(firestore, `/movements/${movId}/users/${myId}`), { uid: myId, role: 'otherRole' })},
          // { label: `Can't ${access.type} if user is not a member of the movement`, assert: assertFails },
          { label: `Can't ${access.type} if user doesn't own the graph`, assert: assertFails, func: () => access.func(theirApp), user: async (firestore) => await setDoc(doc(firestore, `/movements/${movId}/users/${myId}`), { uid: myId, role: 'owner' }) }
        ]
        for(let test of tests) {
          it(test.label, async () => {
            await testEnv.withSecurityRulesDisabled(async (context) => {
  ...
  describe('User Role Definitions', () => {
          ...
          it(test.label, async () => {
            await testEnv.withSecurityRulesDisabled(async (context) => {
              ...
              if(!!test.permissions){
                if(test.label === 'Can create doc if user has permission') {test.permissions}
                await setDoc(doc(firestore, `/movements/${movId}/user-role-definitions/otherRole`), test.permissions)
                // await setDoc(doc(firestore, `/movements/${movId}/user-role-definitions/anotherRole`), test.permissions)
              }
              if(!!test.user){
                await test.user(firestore)
              }
    ...
    describe('Owner doc', () => {
      ...
      for(let access of accessTests) {
        let tests = [
          { label: `Can't ${access.type} doc if user is owner and role is owner`, assert: assertFails, user: async (firestore) => await setDoc(doc(firestore, `/movements/${movId}/users/${myId}`), { uid: myId, role: 'owner' }) },
          { label: `Can't ${access.type} doc if user has permission and role is owner`, assert: assertFails, permissions: { rules: { [access.permission]: true} } , user: async (firestore) => await setDoc(doc(firestore, `/movements/${movId}/users/${myId}`), { uid: myId, role: 'otherRole' })},
  ...
  describe('Requests Doc', () => {
    ...
    for(let access of accessTests) {
      let tests = [
        { label: `Can ${access.type} doc if user is owner`, assert: assertSucceeds, skip: ['create'], user: async (firestore) => await setDoc(doc(firestore, `/movements/${movId}/users/${myId}`), { uid: myId, role: 'owner' }) },
        { label: `Can ${access.type} doc if user has permission`, assert: assertSucceeds, skip: ['create'], permissions: { rules: { [access.permission]: true} } , user: async (firestore) => await setDoc(doc(firestore, `/movements/${movId}/users/${myId}`), { uid: myId, role: 'otherRole' })},
